...
class AnalysisDashboard(QWidget):
    def __init__(self, data_manager, parent=None):
        super().__init__(parent)
        self.data_manager = data_manager
        plt.style.use('seaborn-white')  # Use light-themed style
        self.setup_ui()
    ...
    def setup_ui(self):
        ...
        # Graph selection dropdown
        selection_group = QGroupBox("Graph Selection")
        selection_group.setStyleSheet("color: black; background-color: #f0f0f0;")
        selection_layout = QVBoxLayout()
        ...
        self.graph_combo.setStyleSheet("background-color: #ffffff; color: black; selection-background-color: #2a82da;")
        self.graph_combo.currentIndexChanged.connect(self.change_graph_type)
        ...
        # Ticker selection list
        ticker_group = QGroupBox("Ticker Selection (Select up to 5)")
        ticker_group.setStyleSheet("color: black; background-color: #f0f0f0;")
        ticker_layout = QVBoxLayout()
        ...
        for ticker in tickers:
            item = QListWidgetItem(ticker)
            self.ticker_list.addItem(item)
        self.ticker_list.setStyleSheet("background-color: #ffffff; color: black; selection-background-color: #2a82da;")
        self.ticker_list.itemSelectionChanged.connect(self.update_visualizations)
        ...
        for label in self.metrics_labels.values():
            label.setStyleSheet("color: black; font-weight: bold; padding: 8px; background-color: #f0f0f0; border-radius: 4px;")
    ...
    def plot_portfolio_performance(self):
        ...
        portfolio_history = get_portfolio_history()
        if not portfolio_history:
            ax.text(0.5, 0.5, 'No portfolio history available', horizontalalignment='center', color='black')
            self.chart_fig.patch.set_facecolor('#ffffff')
        ...
        df = df[(df['date'] >= self.start_date) & (df['date'] <= self.end_date)]
        ax.plot(df['date'], df['value'], label='Portfolio Value', color='#2a82da')
        ax.set_title('Portfolio Performance Over Time', color='black')
    ...
    def plot_portfolio_composition(self):
        ...
        orders = get_orders()
        if not orders:
            ax.text(0.5, 0.5, 'No trade history available', horizontalalignment='center', color='black')
            self.chart_fig.patch.set_facecolor('#ffffff')
            ax.set_facecolor('#ffffff')
            return
        ...
        if orders_df.empty:
            ax.text(0.5, 0.5, 'No trades in the selected date range', horizontalalignment='center', color='black')
            self.chart_fig.patch.set_facecolor('#ffffff')
            ax.set_facecolor('#ffffff')
            return
        ...
        total_actions = buy_count + sell_count
        if total_actions == 0:
            ax.text(0.5, 0.5, 'No buy or sell actions recorded', horizontalalignment='center', color='black')
            self.chart_fig.patch.set_facecolor('#ffffff')
            ax.set_facecolor('#ffffff')
            return
        ...
        if non_zero:
            labels, sizes, colors = zip(*non_zero)
            ax.pie(sizes, labels=labels, autopct='%1.1f%%', colors=colors)
            ax.set_title('Trade Action Distribution', color='black')
        else:
            ax.text(0.5, 0.5, 'No trade actions to display', horizontalalignment='center', color='black')
        self.chart_fig.patch.set_facecolor('#ffffff')
        ax.set_facecolor('#ffffff')
    ...
    def plot_portfolio_vs_max_possible(self):
        ...
        if df.empty:
            ax.text(0.5, 0.5, 'No portfolio history in the selected date range', horizontalalignment='center', color='black')
            self.chart_fig.patch.set_facecolor('#ffffff')
            ax.set_facecolor('#ffffff')
            return
        ...
        if data.empty:
            ax.text(0.5, 0.5, 'No data available for the date range', horizontalalignment='center', color='black')
            self.chart_fig.patch.set_facecolor('#ffffff')
            ax.set_facecolor('#ffffff')
            return
        ...
        ax.plot(df['date'], df['value'], label='Actual Portfolio Value', color='#2a82da')
        ax.plot(date_range, max_values, label='Max Possible Value', color='#ff9900', linestyle='--')
        ax.set_title('Portfolio Value vs Maximum Possible Value', color='black')
        ax.set_xlabel('Date', color='black')
        ax.set_ylabel('Value ($)', color='black')
        ax.legend()
        ax.grid(True, color='#cccccc')
    ...
    def plot_sharpe_comparison(self, selected_tickers):
        ...
        ax.set_title('Daily Predicted vs Periodic Actual Sharpe Ratios (Raw)', color='black')
        ax.set_xlabel('Date', color='black')
        ax.set_ylabel('Sharpe Ratio', color='black')
        ax.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        ax.grid(True, color='#cccccc')
        ax.tick_params(colors='black')
        self.chart_fig.patch.set_facecolor('#ffffff')
    ...
    def plot_sharpe_distribution(self, selected_tickers):
        ...
        ax.set_title('Distribution of Sharpe Ratios', color='black')
    ...
    def plot_scatter_with_regression(self, selected_tickers):
        ...
        if filtered_data.empty:
            ax.text(0.5, 0.5, 'No data with Actual Sharpe != -1.0', horizontalalignment='center', color='black')
            self.chart_fig.patch.set_facecolor('#ffffff')
            ax.set_facecolor('#ffffff')
            return
        ...
        combined_data = pd.concat(overall_data)
        sns.regplot(x='Best_Prediction', y='Actual_Sharpe', data=combined_data, ax=ax, 
                    scatter=False, color='#2a82da', label='Regression Line')
        ...
        ax.set_title('Actual Sharpe vs Predicted Sharpe (Raw)', color='black')
        ax.set_xlabel('Predicted Sharpe Ratio (Best_Prediction)', color='black')
        ax.set_ylabel('Actual Sharpe Ratio', color='black')
        ax.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        ax.grid(True, color='#cccccc')
        ax.tick_params(colors='black')
        self.chart_fig.patch.set_facecolor('#ffffff')
    ...
    def plot_time_series_comparison(self, selected_tickers):
        ...
        ax.set_ylim(-5, 5)
        ax.set_title('Time Series Comparison of Sharpe Ratios (Raw)', color='black')
        ax.set_xlabel('Date', color='black')
        ax.set_ylabel('Sharpe Ratio', color='black')
        ax.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        ax.grid(True, color='#cccccc')
    ...
    def plot_error_distribution(self, selected_tickers):
        ...
        ax.set_title('Distribution of Prediction Errors (Raw)', color='black')
        ax.set_xlabel('Prediction Error', color='black')
    ...
    def plot_performance_by_stock(self, selected_tickers):
        ...
        performance.plot(kind='bar', ax=ax, color='#2a82da')
        ax.set_title('Average Prediction Error by Stock', color='black')
        ax.set_xlabel('Stock Ticker', color='black')
        ax.set_ylabel('Mean Absolute Error', color='black')
        ax.grid(True, color='#cccccc')
        ax.tick_params(colors='black')
        self.chart_fig.patch.set_facecolor('#ffffff')
        ax.set_facecolor('#ffffff')
    ...
    def plot_ensemble_heatmap(self, selected_tickers):
        ...
        if len(pred_cols) < 2:
            ...
            if errors.empty:
                ax.text(0.5, 0.5, 'No error data to plot', horizontalalignment='center', color='black')
            else:
                sns.heatmap(errors, ax=ax, cmap='viridis', annot=True, fmt='.2f')
                ax.set_title('Prediction Error by Stock (Best_Prediction)', color='black')
        else:
            performance = {}
            for method in pred_cols:
                errors = np.mean(np.abs(data[method] - data['Actual_Sharpe']))
            ...
            performance_matrix = pd.DataFrame(performance, index=['Mean Error'])
            sns.heatmap(performance_matrix, ax=ax, cmap='viridis', annot=True, fmt='.2f')
            ax.set_title('Ensemble Method Performance (Mean Error)', color='black')
    ...
    def plot_sharpe_accuracy(self, selected_tickers):
        ...
        # Plot actual Sharpe ratios as lines
        actual_data = data[data['Actual_Sharpe'] != -1.0]
        if not actual_data.empty:
            for idx, ticker in enumerate(selected_tickers):
                ticker_data = actual_data[actual_data['Ticker'] == ticker]
